...
export class GroqService {
  ...
  async init(systemPrompt) {
    ...
    // Tenta carregar configuração local primeiro
    try {
      const response = await fetch('./botData/api-config.json?t=' + Date.now());
      if (response.ok) {
        const fileConfig = await response.json();
        console.log('Arquivo carregado:', fileConfig);
        this.config = fileConfig;
        console.log('Configuração carregada do arquivo:', this.config.groq.apiKey.substring(0, 10) + '...');
        return true;
      }
    } catch (error) {
      console.log('Erro ao carregar configuração local:', error);
    }
  ...
  async prompt(text, signal) {
    ...
    const apiUrl = isProduction 
      ? 'https://lukasdevjobs1.vercel.app/api/chat'  // Servidor proxy (produção)
    ...
    try {
      ...
      // Adiciona Authorization se tiver uma chave válida
      if (this.config.groq.apiKey && this.config.groq.apiKey !== 'proxy_mode' && this.config.groq.apiKey.startsWith('gsk_')) {
        headers['Authorization'] = `Bearer ${this.config.groq.apiKey}`;
        console.log('Usando API key local:', this.config.groq.apiKey.substring(0, 10) + '...');
      } else if (isProduction) {
        console.log('Usando servidor proxy (sem API key)');
      } else {
        console.log('Chave API não encontrada ou inválida');
      }
